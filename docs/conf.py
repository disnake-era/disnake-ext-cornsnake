...
version = ""
with Path(
    "../disnake/ext/cornsnake/__init__.py",
).open(encoding=locale.getpreferredencoding(False)) as f:
    matches = re.search(r'^__version__\s*=\s*[\'"]([^\'"]*)[\'"]', f.read(), re.MULTILINE)

    if not matches:
        raise RuntimeError("Could not find version string in disnake/ext/cornsnake/__init__.py")

    version = matches.group(1)

release = version

github_repo_url = "https://github.com/elenakrittik/cornsnake"
